...
PID_FILE=pid_file
APP_LOG=start.log
APP_NAME=redis-demo-1.0.0.jar
JVM_OPTS="-Xms256M -Xmx512M -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=./log/dump/"
AGENT_OPTS="-javaagent:/home/agent/pinpoint-agent-2.4.2/pinpoint-bootstrap.jar -Dpinpoint.agentId=redis-demo-node1 -Dpinpoint.agentName=redis-demo-node1 -Dpinpoint.applicationName=redis-demo"
...
start_app() {
  ...
  nohup ${JAVA_HOME} ${AGENT_OPTS} ${JVM_OPTS} -jar ${APP_NAME} -Dspring.config.location=config/application-cluster.yml --spring.profiles.active=cluster 2>&1 >> ${APP_LOG} &
  echo $! > ${PID_FILE}
}
